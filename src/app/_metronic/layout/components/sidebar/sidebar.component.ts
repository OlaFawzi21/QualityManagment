...
export class SidebarComponent implements OnInit, OnDestroy {
  ...
  updateProps(config: ILayout) {
    const kt_theme_mode_menu = localStorage.getItem('kt_theme_mode_menu');
    const kt_theme_mode_value = localStorage.getItem('kt_theme_mode_value');

    let sidebarMode: string;

    if (kt_theme_mode_value === 'dark') {
      sidebarMode = 'dark-sidebar';
    } else if (kt_theme_mode_value === 'light') {
      sidebarMode = 'light-sidebar';
    } else if (kt_theme_mode_value === 'system') {
      sidebarMode =
        kt_theme_mode_menu === 'dark' ? 'dark-sidebar' : 'light-sidebar';
    } else {
      sidebarMode = 'light-sidebar';
    }

    document.body.setAttribute('data-kt-app-layout', sidebarMode);
