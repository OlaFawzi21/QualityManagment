...
export class PlaceComponent {
  ...
  onAudioSelected(event: any) {
    ...
    if (file && audioTypes.includes(file.type)) {
      ...
      this.messageService.add({
        severity: 'error',
        detail: 'الملف المحدد غير مدعوم، الرجاء تحميل ملف بصيغة mp3 أو wav',
      });
      return;
    }
  }
  ...
  onVideoOrImageSelected(event: any) {
    ...
    if (file) {
      if (this.isImage(file) || this.isVideoAccepted(file)) {
        this.uploadVideoOrImage(file);
      } else {
        this.messageService.add({
          severity: 'error',
          detail: 'الرجاء تحميل صورة أو فيديو بتنسيق مدعوم.',
        });
      }
    }
  }
  ...
  onPdfSelected(event: any, i: number) {
    ...
    if (file && file.type === pdfType) {
      this.uploadPdf(file, i);
    } else {
      this.messageService.add({
        severity: 'error',
        detail: 'الملف المحدد غير مدعوم، الرجاء تحميل ملف بصيغة pdf',
      });
    }
  }
